...
document.getElementById('btn-export').addEventListener('click', () => {
    ...
    const proofUrl = `https://anvil-protocol.com#anvil_verify=${base64Data}`;
    const color = purity >= 80 ? '#00BA7C' : '#FF4444';
    
    // HTML badge for rich text editors
    const htmlBadge = `<a href="${proofUrl}" style="display:inline-block; background-color:${color}15; color:${color}; padding:4px 10px; border-radius:100px; text-decoration:none; font-family:sans-serif; font-weight:bold; border:1px solid ${color}; font-size:12px;">🛡️ Verified Human | ${purity}% Purity</a>`;
    
    // Plain text badge for plain text editors
    const plainBadge = `[ 🛡️ Verified Human | ${purity}% Purity | Anvil Protocol ]`;
