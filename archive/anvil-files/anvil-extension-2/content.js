...
function copyBadge(type) {
    ...
    const html = `<span title="${tooltipText}" data-anvil-verify="${base64}" style="display: inline-block; background-color: ${bgColor}; color: ${color}; padding: 4px 12px; border-radius: 50px; font-family: -apple-system, BlinkMacSystemFont, sans-serif; font-weight: bold; border: 1px solid ${color}; font-size: 12px; line-height: 1.4; vertical-align: middle;">${label}</span>`;
    
    // Plain text fallback (only shown if app doesn't support HTML)
    const plain = `[ ${label} ]`;
