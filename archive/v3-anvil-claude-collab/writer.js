...
document.getElementById('btn-export').addEventListener('click', () => {
    ...
    const color = purity >= 80 ? '#00BA7C' : '#FF4444';
    
    // HTML badge - no external link, data is self-contained in title tooltip
    ...
    const htmlBadge = `<span title="${tooltipText}" data-anvil-verify="${base64Data}" style="display:inline-block; background-color:${color}15; color:${color}; padding:4px 10px; border-radius:100px; font-family:sans-serif; font-weight:bold; border:1px solid ${color}; font-size:12px; cursor:default;">🛡️ Verified Human | ${purity}% Purity</span>`;
    
    // Plain text badge for plain text editors
    const plainBadge = `[ 🛡️ Verified Human | ${purity}% Purity | Typed: ${session.humanChars} | Pasted: ${session.alienChars} ]`;
