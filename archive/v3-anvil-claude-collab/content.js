...
function initBadgeListener() {
    document.addEventListener('click', (e) => {
        // Check for new span-based badges with data attribute
        const spanTarget = e.target.closest('[data-anvil-verify]');
        if (spanTarget) {
            const rawData = spanTarget.getAttribute('data-anvil-verify');
            if (rawData) {
        ...
        // Fallback: Check for legacy link-based badges
        const linkTarget = e.target.closest('a');
        if (linkTarget && linkTarget.href && linkTarget.href.includes('#anvil_verify=')) {
            e.preventDefault();
            const rawData = linkTarget.href.split('#anvil_verify=')[1];
            showCertificate(rawData);
        }
    });
}
...
function updateUI() {
    ...
    menu.innerHTML = `
        <div class="anvil-header">
            ...
            <span class="anvil-close" id="btn-close-menu">×</span>
        </div>
        <div class="anvil-body">
            <div class="anvil-row">
                <span>Rank</span>
            ...
            <div class="anvil-btn" id="btn-open-writer" style="margin-top:8px; background:#1a1a1a; border:1px solid #333;">Open Writer</div>
        </div>
    `;
    ...
    requestAnimationFrame(() => {
        ...
        const writerBtn = document.getElementById('btn-open-writer');
...
function copyBadge(type) {
    ...
    if (type === 'passport') {
        ...
        color = '#FFD700';
        tooltipText = `Anvil Passport | Rank: ${passport.level} | Total Keys: ${passport.totalKeystrokes.toLocaleString()} | Date: ${date}`;
    } else {
        const total = project.humanChars + project.alienChars;
        ...
        tooltipText = `Anvil Verified | Typed: ${project.humanChars} | Pasted: ${project.alienChars} | Purity: ${purity}% | Date: ${date}`;
    }

    // Encode data for potential future verification
    const json = JSON.stringify(payload);
    const base64 = btoa(json);

    // HTML badge - self-contained with tooltip, no external link
    const html = `<span title="${tooltipText}" data-anvil-verify="${base64}" style="display:inline-block; background:${color}15; color:${color}; padding:4px 10px; border-radius:50px; font-family:sans-serif; font-weight:bold; border:1px solid ${color}; font-size:12px; cursor:default;">${label}</span>`;
    
    // Plain text fallback with details
    const plain = `[ ${label} | ${tooltipText} ]`;
