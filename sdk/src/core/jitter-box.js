...
// Paste weighting (JITTER-PLAN.md): a paste's contribution to "alien" chars is
// scaled by ITS OWN size, not lumped together. A short paste (URL, name) is
// noise; a large paste counts fully. Transparent, not punished — only a near-
// total flood trips a hard guard downstream.
...
function weightedAlienChars(pasteSizes, alienChars) {
  ...
  if (pasteSizes && pasteSizes.length) {
    ...
    // Alien chars with no sized paste behind them (e.g. MutationObserver voice/
    // drag input) count at full weight — conservative.
    return round2(sum + Math.max(0, alien - accounted))
  }
  return alien // no per-paste sizes → full weight (backward compatible)
}
...
function scoreProfile(profile, captureData) {
  ...
  // Hard Rule #4: paste is normal (quotes, URLs, names). Small pastes barely
  // move purity; multiple pastes are flagged, not penalized; only a near-total
  // paste flood (>90% weighted) trips the one hard guard against AI laundering.
  var humanChars = (captureData && captureData.humanChars) || 0
  var rawAlien = (captureData && captureData.alienChars) || 0
  var rawTotal = humanChars + rawAlien
  var weightedAlien = weightedAlienChars(captureData && captureData.pasteSizes, rawAlien)
  var effTotal = humanChars + weightedAlien
  var pasteRatio = effTotal > 0 ? weightedAlien / effTotal : 0
  if (rawTotal >= MIN_CHARS_FOR_SCORE && captureData) {
    components.purity = rampScore(effTotal > 0 ? humanChars / effTotal : 1, RAMPS.purity[0], RAMPS.purity[1])
    var pasteCount = (captureData.pasteSizes && captureData.pasteSizes.length) || 0
...
function attach(el) {
  ...
  function onPaste(e) {
    ...
    if (pasted.length > 0) {
      ...
      data.pasteSizes.push(pasted.length) // per-paste size drives size-weighted purity
    }
  }
    ...
    if (lastInputTime > 0) {
      ...
      if (flight >= MIN_FLIGHT_MS && flight <= MAX_FLIGHT_MS) {
        ...
        if (data.totalKeystrokes > 0 && data.totalKeystrokes % 25 === 0) {
          var recent = data.flightTimes.slice(-25)
          var avg = 0
          for (var j = 0; j < recent.length; j++) avg += recent[j]
          avg = avg / recent.length
          data.fatigueWindows.push(round2(avg))
          if (data.fatigueWindows.length > FATIGUE_WINDOW_COUNT) data.fatigueWindows.shift()
        }
      }
    }
