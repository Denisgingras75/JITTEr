// Runs the real attest, verify and erase functions locally against a PGlite
// database with the repo migrations applied, on http://127.0.0.1:54321/functions/v1/*.
...
const routes: Record<string, (req: Request) => Response | Promise<Response>> = {
  '/functions/v1/attest': attestH,
  '/functions/v1/verify': verifyH,
  '/functions/v1/erase': eraseH,
};

const port = Number(Deno.env.get('PORT') || 54321);
Deno.serve({ hostname: '127.0.0.1', port, onListen: () => console.log(`functions listening on http://127.0.0.1:${port}/functions/v1/{attest,verify,erase}`) }, async (req) => {
